...
byte_to_sector (const struct inode *inode, off_t pos) 
{
  ...
  else if (sectors < (DIRECT_PTRS + ENTRIES + ENTRIES*ENTRIES)) 
  {// if indexed by double indirection
...
/* Inode Create Helper
  What it does: Attempts to create a contiguous block of disk sectors, in the interest of locality.
    HOWEVER! It if fails, falls back to incremental allocation, so that if N blocks are free, no matter
    the arrangement, N blocks can be allocated.
  What it takes: number of blocks to allocate, number of metadata structures, a pointer to a zeroed page,
    and a pointer to the RAM representation of an inode (inode_disk)
  What it returns: A boolean representing success.
*/
  bool
  inode_create_helper (uint32_t inode_blocks, uint32_t data_blocks, char* zeros, struct inode_disk* ram_inode )
  {
    ...
    {
      DEBUGMSG("NO CONTIGUOUS BLOCKS\n");
      return incremental (inode_blocks, data_blocks, zeros, ram_inode);
    }
  }

/* incremental
  What it does: incrementally allocates blocks, so that sectors need not be contiguous.
  What it takes: number of blocks to allocate, number of metadata structures, a pointer to a zeroed page,
    and a pointer to the RAM representation of an inode (inode_disk)
  What it returns: A boolean representing success.
*/
  bool incremental ( uint32_t inode_blocks, uint32_t data_blocks, char* zeros, struct inode_disk* master_inode )
  {
    ...
    if(!data_blocks_left)
    {// only direct, and they've all been allocated
    ...
    {
      if(!(i%ENTRIES))
      { // if it requires creating a single_indirection
        if (!free_map_allocate(1,  &block_allocated)){
          free (indr);
          free (db_indr);
          return false;
...
/* contiguous
  What it does: contiguously allocates blocks if possible, to exploit locality.
  What it takes: number of blocks to allocate, number of metadata structures, a pointer to a zeroed page,
    and a pointer to the RAM representation of an inode (inode_disk)
  What it returns: A boolean representing success.
*/
...
/* write_inode_to_sector
  What it does: writes given inode to given sector
  What it takes: inode pointer to write, sector to write it to
...
/* extend_data_sector
  what it takes: inode_disk pointer of RAM inode, block_sector t index into inode_disk structure, free sector where the sector_idx actually lives
  what it returns: nothing
*/
void extend_data_sector(struct inode_disk* inode, block_sector_t sector_idx, block_sector_t free_sector_idx)
{
  DEBUGMSG("EDS: ");
  static char zeros[BLOCK_SECTOR_SIZE];
